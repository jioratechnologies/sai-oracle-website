...
/**
 * Magic UI's "Pulsating Button" — a box-shadow ring that pulses or
 * ripples outward from the button's own background color, to draw the
 * eye to one high-value action (WhatsApp / call-to-action).
 */
export const PulsatingButton = React.forwardRef<HTMLButtonElement, PulsatingButtonProps>(
  (
    { className, children, pulseColor, duration = "1.5s", distance = "8px", variant = "pulse", ...props },
    ref,
  ) => {
    ...
    return (
      <button
        ref={innerRef}
        className={cn(
          "relative inline-flex cursor-pointer items-center justify-center rounded-full bg-maroon-800 px-6 py-3 text-center font-bold text-white shadow-lg transition-transform active:scale-[0.98]",
          className,
        )}
        style={
          {
      ...
      >
        ...
        <span
          ...
          className={cn(
            ...
            variant === "pulse" ? "pulsating-btn-pulse" : "pulsating-btn-ripple",
          )}
        />
      </button>
    );
