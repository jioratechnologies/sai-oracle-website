    ...
    <section className="section-dawn relative overflow-hidden text-stone-900">
      {image ? (
        <>
          ...
          <div
            aria-hidden
            className="absolute inset-0 bg-linear-to-b from-cream-50/85 via-cream-50/70 to-cream-50"
          />
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-9 text-center sm:py-12">
        <nav aria-label="Breadcrumb">
          <ol className="mb-4 flex flex-wrap items-center justify-center gap-1.5 text-[13px] text-stone-500">
            <li>
              <Link href="/" className="transition-colors hover:text-saffron-600">
                Home
              </Link>
            </li>
            {trail.map((c) => (
              <li key={c.label} className="flex items-center gap-1.5">
                <span aria-hidden className="text-saffron-500/70">
                  <ChevronRight className="h-3.5 w-3.5" />
                </span>
                {c.href ? (
                  <Link href={c.href} className="transition-colors hover:text-saffron-600">
                    {c.label}
                  </Link>
                ) : (
                  <span aria-current="page" className="font-medium text-saffron-700">
                    {c.label}
                  </span>
                )}
              </li>
        ...
        {eyebrow && (
          <p className="mb-3 flex items-center justify-center gap-2.5 text-[11px] font-bold tracking-[0.3em] text-saffron-700 uppercase">
        ...
        <TextReveal
          as="h1"
          text={title}
          className="font-display text-[2.1rem] leading-tight font-extrabold text-balance text-red-700 sm:text-[2.75rem]"
        />
        {intro && (
          <p className="mx-auto mt-3 max-w-2xl text-[15px] leading-relaxed text-stone-600 sm:text-base">
            {intro}
          </p>
        )}
      </div>
      ...
      <div aria-hidden className="relative divider-festive">
        <span className="absolute top-1/2 left-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rotate-45 border-2 border-saffron-500 bg-cream-50" />
      </div>
    </section>
  );
}
